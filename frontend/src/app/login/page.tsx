...
import { Sparkles, Mail, ShieldAlert, Layers } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleDeveloperLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || "Failed to generate mock session.");
        setLoading(false);
      }
    } catch (err: any) {
      setError("Connection to mock login API failed.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-950 text-zinc-100 font-sans antialiased selection:bg-blue-600/30 selection:text-blue-200 overflow-hidden relative">
      ...
      <main className="relative z-10 w-full max-w-md px-6 py-12 animate-fade-in">
        {/* Brand Header */}
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-white mb-2">
            Sign in to{" "}
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-indigo-400 to-cyan-400 animate-pulse">
              RepoProof
            </span>
          </h1>
        ...
        <div className="rounded-2xl border border-zinc-900 bg-zinc-900/30 backdrop-blur-xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleDeveloperLogin} className="space-y-4">
            ...
            >
              {loading ? "Signing in..." : "Developer Login Bypass"}
            </button>
          </form>
        </div>
